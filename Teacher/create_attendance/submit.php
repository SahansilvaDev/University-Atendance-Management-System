...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($courseQuery && $courseCode && $batchYearQuery) {
        ...
        $batchYearText = $batchYearData['year'] . " - " . $batchYearData['batch'];
         ...
         $sql_sa = "INSERT INTO t_success_attend (user_id, subject_select, subject_code, batch, qr_date, qr_code) VALUES ('$user_id', '$courseName', '$courseCode', '$batchYearText1', '$qr_date', '$qr_code')";
         $sa_result = mysqli_query($conn, $sql_sa);


        // Prepared statement to insert data
        $sql_i = "INSERT INTO create_qr_attendance (subject_select, subject_code, batch, batch_year, mentrol_or_interctive, time, qr_date, start_time, end_time, subject_time, qr_code, expired, teacher_id) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        if ($stmt === false) {
            ...
            $stmt->bind_param("ssssissssssis", $courseName, $courseCode, $batchYearText1, $batchYearText, $switch_btn, $time, $qr_date, $start_time, $end_time, $subject_time, $qr_code, $expired, $user_id);
            ...
            if ($stmt->execute()) {
                ...
                <script>
                    if (confirm("Data sent Successfully")) {
                        window.location.href = "../create_Attendance.php";
                    }
                </script>
                <?php
            } else {
...
<script>
    Swal.fire({
        ...
        icon: 'success',
        confirmButtonText: '<a href="../create_Attendance.php">OK</a>'
    }).then((result) => {
