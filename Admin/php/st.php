...
if (isset($_POST['stu_submit'])) {
    ...
    //     $stmt = $conn->prepare("INSERT INTO student (user_id, fname, lname, name, email, dob, nic, phone_number, address, password, token) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    //     $stmt->bind_param("sssssssssss", $user_id, $fname, $lname, $name, $email, $dob, $nic, $phone_number, $address, $hashed_password, $message);
    //     $stmt->execute();
    //     $stmt->close();
