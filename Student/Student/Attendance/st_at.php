...
if (mysqli_num_rows($time_result) > 0) {
    ...
    // }
        ...
        if (!$qr_check_result) {
            die("Error retrieving QR code details: " . mysqli_error($conn));
        }

        if (mysqli_num_rows($qr_check_result) > 0) {
            $qr_row = mysqli_fetch_assoc($qr_check_result);
            $expired = $qr_row['expired'];
            $qr_code = $qr_row['qr_code'];
            $subject_time = $qr_row['subject_time'];

           
            $start_time = strtotime($qr_row['time']);
            $stop_time = $start_time + ($subject_time * 60);

            

 
            $formatted_start_time = date('h:i:s A', $start_time);
            $formatted_stop_time = date('h:i:s A', $stop_time);

          
            $current_time_seconds = time();

            if ($current_time_seconds < $stop_time) {
               
                $remaining_time = ($stop_time - $current_time_seconds) * 1000;

               
                echo "<script>";
                echo "var remainingTime = $remaining_time;";
                echo "</script>";
?>
                ...
                <script>
                    ...
                    var stopTime = <?php echo $stop_time; ?>;
...
<?php
        ...
        // Display message if current time does not match the desired time
        echo "<h4 class='my-5 py-5'>Desired time does not match the current time</h4>";
    }
// } else {
  
//     echo "<h4 class='my-5 py-5'>No time record found for today</h4>";
// }
}
