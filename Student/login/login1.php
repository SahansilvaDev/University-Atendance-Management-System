...
$sessionTimeout = 1; // Change this to your desired session timeout
$_SESSION['timeout'] = $sessionTimeout * 60; // Convert minutes to seconds
...
if (isset($_POST['user_id'], $_POST['password'])) {
    ...
    $stmt = $conn->prepare("SELECT user_id, name, password, login_ip FROM student WHERE user_id=?");
    ...
    if ($result->num_rows > 0) {
        ...
        if (password_verify($password, $row['password'])) {
            ...
            // Capture and store login IP
            $login_ip = $_SERVER['REMOTE_ADDR'];

            // Update the database with the login IP
            $updateStmt = $conn->prepare("UPDATE student SET login_ip = ? WHERE user_id = ?");
            $updateStmt->bind_param("ss", $login_ip, $row['user_id']);
            $updateStmt->execute();
        ...
        header("Location: ./login1.php?error=3");
        exit();
    }
}
