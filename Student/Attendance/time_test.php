...
<!-- <div class="row">
    ...
    <div class="col-md-2"></div>
                                ...
                                <?php
// Get subject_time from create_qr_attendance table
$query_subject_time = "SELECT subject_time, qr_code FROM create_qr_attendance WHERE qr_code = '$qr_code'";
$result_subject_time = mysqli_query($conn, $query_subject_time);

if ($result_subject_time) {
    $row_subject_time = mysqli_fetch_assoc($result_subject_time);
    $subject_time = $row_subject_time['subject_time'];
    $subject_time_qr_code = $row_subject_time['qr_code'];
} else {
    // Handle error
    echo "Error fetching subject_time: " . mysqli_error($conn);
}
?>
...
<script>
    ...
    function updateTime() {
        ...
        // Set the countdown duration to subject_time minutes (in milliseconds)
        ...
        if (remainingTime <= 0) {
            <?php
                ...
                $time_up = 'document.getElementById("time").innerHTML = "Time\'s up!"; ';
                ...
                if (time() >= $time_limit) {
                    ...
                    $automatic_update_query = "UPDATE create_qr_attendance SET expired = 1 WHERE qr_code = '$qr_code'";
                    $automatic_update_result = mysqli_query($conn, $automatic_update_query);
...
$query_expired = "SELECT expired, qr_code FROM create_qr_attendance WHERE qr_date = '$qr_date'";
$result_expired = mysqli_query($conn, $query_expired);

if ($result_expired) {
    $row_expired = mysqli_fetch_assoc($result_expired);
    $expired = $row_expired['expired'];
    $qr_code = $row_expired['qr_code'];
